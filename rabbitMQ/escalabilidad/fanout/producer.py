...
class IoTDevice:
    ...
    def connect(self):#esto se modifico, explicar
        self.connection = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
        self.channel = self.connection.channel()
        self.channel.exchange_declare(exchange='iot_exchange', exchange_type='fanout')
        result = self.channel.queue_declare(queue='', exclusive=True)
        queue_name = result.method.queue
        self.channel.queue_bind(exchange='iot_exchange', queue=queue_name)
    ...
    def send_data(self):
        ...
        self.channel.basic_publish(exchange='iot_exchange', #explicar
                                   routing_key='',
                                   body=json_data)
